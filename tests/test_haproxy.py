...
def test_harproxy_update_config_supports_resolver(tmpdir, mocker):
    ...
    assert "server foo-foo-blue1 10.0.0.1:80 check inter 10s weight 100  resolvers my-resolver" in updated_config
...
def test_harproxy_update_config_supports_ssl_backend_failover(tmpdir, mocker):
    ...
    assert "server foo-foo-blue-backup bar.example.com backup check inter 10s  resolvers    ssl ca-file " \
           "ca-certificates.crt" in updated_config
...
def test_harproxy_update_config_supports_ssl_backend_failover_with_self_signed_cert(tmpdir, mocker):
    ...
    updated_config = config.read()
    assert "server foo-foo-blue-backup bar.example.com backup check inter 10s  resolvers    ssl ca-file " \
           "ca-certificates.crt   verify none" in updated_config
